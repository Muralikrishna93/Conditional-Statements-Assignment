...
function loginUser(userName = "Guest", password = "000000") {
...
function generateInvoice() {
  ...
  const { discount, finalPrice } = applyDiscount(cartTotal);
  ...
  cart.forEach((item) => {
    console.log(` ${item.name} | Subtotal: ${item.total}`);
  }
  );
...
processPayment(finalPrice, paymentSuccess);
